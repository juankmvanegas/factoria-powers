...
1. **Identify logic to verify** — Select the migrated module or use case. Load both:
   - Legacy source code (original Django/Flask implementation)
   - New implementation (Clean Architecture use case)

2. **Extract test scenarios from legacy** — Analyze legacy code for:
   - Happy path flows (normal input → expected output)
...
3. **Create equivalence tests** — For each scenario:
...
4. **Test data transformations** — Verify:
...
5. **Test edge cases** — Specifically verify:
...
6. **Generate verification report** — Output to `.cloud/planning/logic-verification-{module}.md`:
