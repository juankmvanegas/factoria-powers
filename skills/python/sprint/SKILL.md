...
1. **Understand the task** — Read the user's description. If ambiguous, ask ONE clarifying question (maximum). Identify affected layer(s) and file(s).

2. **Load context** — Read the affected files and their tests. Load relevant policy if the change touches security, testing, or coding standards.

3. **Implement the fix** — Make the minimal change required. Follow coding standards policy. Respect layer boundaries even for small changes.

4. **Validate** — Run affected tests (`pytest tests/ -k {relevant_test}`). Run `ruff check` on changed files. Run `mypy` on changed files. If any test fails, fix and re-run.

5. **Chain completion** — Automatic post-execution:
