...
1. **Identify rollback scope** — Determine what needs to be rolled back:
...
2. **Generate git rollback strategy** — Based on the changes:
...
3. **Generate database rollback steps** — Based on Alembic migrations:
...
4. **Generate service restart steps** — Document:
...
5. **Generate verification checklist** — After rollback:
...
6. **Write rollback plan** — Save to `.cloud/planning/rollback-{change-slug}.md`:
...
- NEVER use `git reset --hard` on shared branches — use `git revert`
