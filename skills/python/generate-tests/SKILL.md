...
1. **Identify target** — Determine what needs testing:
...
2. **Analyze the code** — Read the target code. Identify:
   - Dependencies (ports) that need mocking
   - Input/output types (DTOs, schemas)
   - Edge cases and error paths
   - Business rules and validation logic

3. **Generate unit tests** — For use cases and domain logic:
...
5. **Generate architecture tests** — For import contract validation:
...
6. **Run and validate** — Execute the full test suite:
   - `pytest tests/unit/ -v` — all unit tests pass
   - `pytest tests/integration/ -v` — all integration tests pass
   - `pytest tests/architecture/ -v` — all architecture tests pass
...
- Test files MUST mirror source structure: `src/app/use_cases/create_user.py` → `tests/unit/use_cases/test_create_user.py`
- Use `conftest.py` for shared fixtures — do NOT duplicate setup across test files
