...
1. **Pre-flight check** — Before any deletion:
...
2. **Inventory what will be removed** — List all Factoria files:
   - `.claude/` — skills, commands, hooks, PRPs
   - `.ai/` — agents, AI skills
   - `.cloud/` — policies, ADRs, architecture docs, planning
   - `CLAUDE.md` — project configuration
   - Display the complete list to the user

3. **Preserve project-specific artifacts** — Before deletion, copy to a backup:
...
4. **Request explicit confirmation** — Show the user:
...
5. **Execute removal** — Delete in order:
...
6. **Verify clean state** — After removal:
...
- This is a ONE-WAY operation — there is no "undo" (except git revert)
