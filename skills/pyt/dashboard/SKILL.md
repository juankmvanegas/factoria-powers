...
1. **Gather progress data** â€” Scan project planning files:
...
2. **Calculate metrics** â€” For each active work stream:
   - Total phases / completed phases â†’ completion percentage
   - Current phase and its status
   - Blocked phases and blocker descriptions
   - Time elapsed since work started
   - Estimated remaining effort

3. **Build dashboard view** â€” Format as structured text:
   ```
   ===== FACTORIA DASHBOARD =====
...
5. **Provide recommendations** â€” Based on current state:
...
- NEVER modify progress data â€” dashboard is READ-ONLY
- NEVER fabricate progress percentages â€” calculate from actual phase status
- ALWAYS read the latest files, do not cache stale data
- ALWAYS show blockers prominently

## Rules

- Dashboard is strictly READ-ONLY â€” it displays, never modifies
