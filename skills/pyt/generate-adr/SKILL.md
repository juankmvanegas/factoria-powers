---
name: pyt-generate-adr
description: "Use when an architectural decision needs to be formally documented â€” new technology, framework change, new layer dependency, or new coding convention"
---
...
1. **Determine next ADR number** â€” Scan `.cloud/architecture/decisions/` for existing ADRs. Find the highest number and increment by 1.

2. **Interview the user** â€” Gather decision context:
...
3. **Generate ADR document** â€” Write to `.cloud/architecture/decisions/ADR-{NNN}-{slug}.md`:
   ```markdown
   # ADR-{NNN}: {Title}
...
4. **Cross-reference** â€” Check if the new ADR affects or supersedes existing ADRs. If so, update the status of affected ADRs to "Superseded by ADR-{NNN}".

5. **Verify indexing** â€” Ensure the new ADR will be picked up by the DocRegistry. Verify the filename follows the pattern `ADR-{NNN}-{kebab-case-slug}.md`.
...
- ADRs are append-only â€” once accepted, the Decision section is immutable
