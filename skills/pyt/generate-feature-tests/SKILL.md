---
name: pyt-generate-feature-tests
...
## Execution Flow Ã¢â‚¬â€ 7 Strict Steps
...
- NEVER use `asyncio.run()` inside tests Ã¢â‚¬â€ use `@pytest.mark.asyncio`
- NEVER share state between tests Ã¢â‚¬â€ each test is independent
- ONE behavior per test Ã¢â‚¬â€ no multi-assertion tests unless verifying the same behavior
