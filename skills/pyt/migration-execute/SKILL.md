...
1. **Load module plan** â€” Read `.cloud/planning/migration-plan.md`. Identify the next approved, unexecuted module. Load its source â†’ target mapping.

2. **Read legacy source** â€” Load all source components for this module. Understand the business logic, data flow, and edge cases.

3. **Create domain layer** â€” Following the mapping:
...
4. **Create application layer** â€” Following the mapping:
...
5. **Create infrastructure layer** â€” Following the mapping:
...
6. **Create API layer** â€” Following the mapping:
...
7. **Test and validate** â€” Comprehensive testing:
...
8. **Update progress** â€” Mark module as "done" in migration plan. Update `.cloud/planning/progress-migration.md`. Document any deviations or decisions made.

## Auto-Shielding

- NEVER migrate more than one module per invocation
- NEVER skip the business logic equivalence verification
- NEVER proceed if tests fail â€” fix first
...
- One module per execution â€” no exceptions
- Layer order is IMMUTABLE: domain â†’ application â†’ infrastructure â†’ api â†’ tests
