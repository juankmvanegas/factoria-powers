...
- Create or update **entities** in `domain/entities/` â€” pure Python classes, dataclasses, or Pydantic base models with no framework dependencies
- Define **port interfaces** in `domain/ports/` â€” abstract base classes (ABC) for repositories, external services
...
- Create **use case** in `application/use_cases/` â€” single class with `async execute()` method
- Define **DTOs** in `application/dtos/` â€” Pydantic models for input/output
...
- Implement **adapters** in `infrastructure/adapters/` â€” concrete implementations of domain ports
- Add **persistence** in `infrastructure/persistence/` â€” SQLAlchemy models, repository implementations
- Configure **dependency wiring** in `infrastructure/config/` â€” FastAPI dependency injection setup
- Validate: implements domain port interfaces

### Step 4: API Layer

- Create **router** in `api/routers/` â€” FastAPI router with endpoint definitions
- Define **schemas** in `api/schemas/` â€” request/response Pydantic models (API-specific)
- Wire **dependencies** in `api/dependencies.py` â€” FastAPI `Depends()` for use case injection
- Register router in `main.py`
- Validate: uses application DTOs, never accesses infrastructure directly

### Step 5: Tests

- **Unit tests** â€” test use case with mocked ports using `AsyncMock`
- **Integration tests** â€” test endpoint with `httpx.AsyncClient` and test database
- **Architecture tests** â€” verify import-linter contracts still pass
