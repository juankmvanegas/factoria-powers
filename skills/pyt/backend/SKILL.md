...
---
name: backend
description: "Auto-skill for Python/FastAPI backend â€” applies Clean Architecture 4 layers, DI, Pydantic, and enterprise standards"
...
1. **Domain** (`domain/`) â€” Entities, value objects, domain events, repository protocols. ZERO external dependencies.
2. **Application** (`application/`) â€” Use cases, DTOs (Pydantic BaseModel), port interfaces (Protocol). Depends only on Domain.
3. **Infrastructure** (`infrastructure/`) â€” Repository implementations, external service adapters, ORM models. Depends on Domain + Application.
4. **Presentation** (`presentation/`) â€” FastAPI routers, request/response schemas, middleware. Depends on Application only.
...
- NEVER import from Infrastructure in Presentation â€” use DI
...
- Use `structlog` for all logging â€” NEVER use `print()` or stdlib `logging` directly
...
```
DomainError (base)
â”œâ”€â”€ BusinessError â€” business rule violations
â”œâ”€â”€ NotFoundError â€” entity not found
â”œâ”€â”€ ValidationError â€” domain validation failures
â”œâ”€â”€ ConflictError â€” concurrent modification / duplicate
â””â”€â”€ AuthorizationError â€” permission denied
```
